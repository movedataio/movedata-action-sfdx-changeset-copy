...
const { getAllSFDXCommands } = __nccwpck_require__(7182);
...
const _createCommand = (cmdId, cmdName, cmdFile) => (flags, opts) => new Promise((resolve, reject) => {
  ...
  cmd.run(cmdArgs.argv)
    ...
    .catch((sfdxErr) => {
      ...
      const errors = __nccwpck_require__(4994);
...
const processAllErrors = (sfdxErrors) => {
  ...
  if (Array.isArray(sfdxErrors)) {
    errors = sfdxErrors.map(currError => processError(currError));
  } else {
...
const sfdx = __nccwpck_require__(1739);
...
async function run(request) {
  ...
  const r = await execCommand('npm install');
  console.log(r);
