    ...
    parseDeployDiagnostic(component, message) {
        ...
        switch (typeName) {
            ...
            default:
                ...
                throw new core_1.SfError(`Unable to parse deploy diagnostic with string message: ${message}`);
        }
    }
...
const parseDefault = (component, message) => {
    ...
    const problem = getProblemFromMessage(message);
    if (!problemType) {
        throw new core_1.SfError(`Unable to parse deploy diagnostic with empty problem type: ${message.toString()}`);
    }
    const diagnostic = {
        error: problem,
        problemType,
...
const getProblemFromMessage = (message) => {
    const problem = typeof message === 'string' ? message : message.problem;
    if (!problem) {
        throw new core_1.SfError(`Unable to parse deploy diagnostic with empty problem: ${message.toString()}`);
    }
    return problem;
};
