...
const getContentsOfReplacementFile = async (path) => {
    if (!fileContentsCache.has(path)) {
        try {
            ...
            throw new core_1.SfError(`The file "${path}" specified in the "replacements" property of sfdx-project.json could not be read.`);
        }
    }
...
const getReplacements = async (cmp, replacementConfigs = []) => {
    ...
    filenames.map(async (f) => [
        ...
        await Promise.all(replacementConfigs
            ...
            toReplace: r.stringToReplace ? (0, exports.stringToRegex)(r.stringToReplace) : new RegExp(r.regexToReplace, 'g'),
