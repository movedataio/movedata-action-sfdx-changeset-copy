...
class Delete extends deployCommand_1.DeployCommand {
    ...
    async delete() {
        ...
        cs.apiVersion = this.getFlag('apiversion') ?? (await this.org.retrieveMaxApiVersion());
        cs.sourceApiVersion = this.getFlag('apiversion') ?? (await this.getSourceApiVersion());
        ...
        if (sourcepaths) {
            ...
            this.components
                .filter((comp) => comp.type.strategies?.adapter === 'bundle' && comp instanceof source_deploy_retrieve_1.SourceComponent)
                .map((bundle) => {
    ...
    async deleteFilesLocally() {
        if (!this.getFlag('checkonly') && this.deployResult?.response?.status === source_deploy_retrieve_1.RequestStatus.Succeeded) {
            const promises = [];
            this.components.map((component) => {
