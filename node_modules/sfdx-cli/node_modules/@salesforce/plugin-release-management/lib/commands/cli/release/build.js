...
const messages = core_2.Messages.loadMessages('@salesforce/plugin-release-management', 'cli.release.build');
class build extends sf_plugins_core_1.SfCommand {
    ...
    async run() {
        ...
        if (isPrerelease) {
            this.log(`NOTE: The release channel '${flags['release-channel']}' is not one of 'latest', 'latest-rc', 'nightly'. It will released as a prerelease.`);
        }
        const auth = pushChangesToGitHub
            ? (0, ts_types_1.ensureString)(new kit_1.Env().getString('GH_TOKEN') ?? new kit_1.Env().getString('GITHUB_TOKEN'), 'The GH_TOKEN env var is required to push changes to GitHub. Use the --build-only flag to skip GitHub operations (a manual push will then be needed)')
            : undefined;
        ...
        const baseBranch = flags['release-channel'] !== 'nightly' && pushChangesToGitHub ? await this.createAndPushBaseBranch(ref) : 'main';
        const repo = await repository_1.PackageRepo.create({ ux: new sf_plugins_core_1.Ux({ jsonEnabled: this.jsonEnabled() }) });
        ...
        const nextVersion = repo.package.determineNextVersion(flags.patch, isPrerelease ? flags['release-channel'] : undefined);
        ...
        if (flags.only) {
            ...
            if (!bumped.length) {
                throw new core_2.SfError('No version changes made. Confirm you are passing the correct dependency and version to --only.');
            }
        }
        else {
...
> Once your PR is ready to be released, merge it into \`${baseBranch}\`.`;
    ...
    async getGithubRef(githubRef, distTag) {
        let ref;
        if (githubRef) {
            ...
            const temp = await repository_1.PackageRepo.create({ ux: new sf_plugins_core_1.Ux({ jsonEnabled: this.jsonEnabled() }) });
            const version = temp.package.getDistTags(temp.package.packageJson.name)[distTag];
            ref = version;
        }
        return ref;
    }
    async createAndPushBaseBranch(ref) {
    ...
    async exec(command, silent = false) {
        try {
            ...
            throw new core_2.SfError(err.message);
        }
    }
    async maybeSetGitConfig(octokit) {
        ...
        if (!username || !email) {
            ...
            if (!username)
                await this.exec(`git config user.name "${user.data.name}"`);
            if (!email)
                await this.exec(`git config user.email "${user.data.email}"`);
        }
    }
}
exports.default = build;
build.description = messages.getMessage('description');
build.summary = messages.getMessage('description');
build.examples = messages.getMessages('examples');
build.aliases = ['cli:latestrc:build'];
build.flags = {
    'start-from-npm-dist-tag': sf_plugins_core_1.Flags.string({
    ...
    'start-from-github-ref': sf_plugins_core_1.Flags.string({
        summary: messages.getMessage('flags.startFromGithubRef'),
        char: 'g',
        exactlyOne: ['start-from-npm-dist-tag', 'start-from-github-ref'],
    }),
    'release-channel': sf_plugins_core_1.Flags.string({
        summary: messages.getMessage('flags.releaseChannel'),
        char: 'c',
        required: true,
    }),
    'build-only': sf_plugins_core_1.Flags.boolean({
        summary: messages.getMessage('flags.buildOnly'),
        default: false,
    }),
    resolutions: sf_plugins_core_1.Flags.boolean({
        summary: messages.getMessage('flags.resolutions'),
        default: true,
        allowNo: true,
    }),
    only: (0, sf_plugins_core_1.arrayWithDeprecation)({
        summary: messages.getMessage('flags.only'),
    }),
    'pinned-deps': sf_plugins_core_1.Flags.boolean({
        summary: messages.getMessage('flags.pinnedDeps'),
        default: true,
        allowNo: true,
    }),
    jit: sf_plugins_core_1.Flags.boolean({
    ...
    patch: sf_plugins_core_1.Flags.boolean({
